...
// GET /families/:id/invitations - Get all invitations for a family
router.get(
  '/:id/invitations',
  authMiddleware,
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (userId === memberId) {
        res.status(400).json({ error: 'Cannot remove yourself as owner. Delete the family instead.' });
        return;
      }
...
// GET /families/:id/recommendations - Get recommendations for family members
router.get(
  '/:id/recommendations',
  authMiddleware,
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // Get recommendations between family members
      const recommendations = await prisma.recommendation.findMany({
        where: {
          OR: [
            {
              recommendedBy: { in: memberIds },
              recommendedTo: userId,
            },
            {
              recommendedBy: userId,
              recommendedTo: { in: memberIds },
            },
          ],
        },
        include: {
