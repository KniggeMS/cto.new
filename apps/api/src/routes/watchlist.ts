...
const createWatchlistEntrySchema = z.object({
  ...
  tmdbType: z.enum(['movie', 'tv'], { errorMap: () => ({ message: 'Type must be either "movie" or "tv"' }) }),
...
const updateWatchlistEntrySchema = z.object({
...
const findEntryForUser = async (entryId: string, userId: string, res: Response, forbiddenMessage: string) => {
...
router.get('/', authMiddleware, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const allowedSortFields: Array<keyof Prisma.WatchlistEntryOrderByWithRelationInput> = ['dateAdded', 'dateUpdated', 'status', 'rating'];
    const sortField = typeof sortBy === 'string' && allowedSortFields.includes(sortBy as keyof Prisma.WatchlistEntryOrderByWithRelationInput)
...
router.patch('/:id', authMiddleware, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const entry = await findEntryForUser(entryId, userId, res, 'You do not have permission to update this entry');
    if (!entry) {
      return;
    }
...
router.patch('/:id/progress', authMiddleware, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.user!.id;
    const entryId = req.params.id;
...
router.delete('/:id', authMiddleware, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const entry = await findEntryForUser(entryId, userId, res, 'You do not have permission to delete this entry');
    if (!entry) {
      return;
    }
